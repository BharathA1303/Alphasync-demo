...
async def broker_master_status():
    ...
    if not master_session_service.is_active():
        # Determine why master session is not active
        error_reason = None
        
        if not settings.ZEBU_MASTER_USER_ID:
            error_reason = (
                "Master Zebu not configured. "
                "Set ZEBU_MASTER_USER_ID in .env to enable live market data."
            )
        elif not settings.ZEBU_MASTER_PASSWORD:
            error_reason = (
                "Master Zebu password not configured. "
                "Set ZEBU_MASTER_PASSWORD in .env"
            )
        elif not settings.ZEBU_MASTER_DOB:
            error_reason = (
                "Master Zebu DOB/TOTP not configured. "
                "Set ZEBU_MASTER_DOB in .env (format: DD-MM-YYYY or 6-digit TOTP)"
            )
        elif not settings.ZEBU_API_SECRET:
            error_reason = (
                "Zebu API Secret not configured. "
                "Set ZEBU_API_SECRET in .env"
            )
        else:
            error_reason = (
                "Master Zebu connection failed. "
                "Check credentials in .env or see backend logs for details."
            )
        ...
        return {
            ...
            "error": error_reason,
            "details": {
                "configured": bool(settings.ZEBU_MASTER_USER_ID),
                "user_id": settings.ZEBU_MASTER_USER_ID[:3] + "*" * 5 if settings.ZEBU_MASTER_USER_ID else None,
            }
        }
