...
class ZebuProvider(MarketProvider):
    ...
    async def _resolve_symbol(self, canonical: str) -> Optional[dict]:
        ...
        """
        # Extract base symbol: "GODREJCP.NS" → "GODREJCP"
        base = canonical.split(".")[0].upper()
        try:
            data = await self._rest_post("/SearchScrip", {"exch": "NSE", "stext": base})
            if not data or data.get("stat") != "Ok":
                return None
            for item in data.get("values", []):
                tsym = item.get("tsym", "")
                token = item.get("token", "")
                # Match exact EQ variant (avoid OPTIONS/FUTURES/partial matches)
