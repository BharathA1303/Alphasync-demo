...
class PortfolioRecalcWorker:
    ...
    async def _handle_sell(
        ...
        if not holding:
            logger.error(f"No holding found for SELL: {symbol} — skipping portfolio update")
            return

        # Clamp quantity to what is actually held (safety guard)
        actual_qty = min(quantity, holding.quantity)
        if actual_qty <= 0:
            logger.error(f"No shares to sell for {symbol} (holding qty={holding.quantity})")
            return
        if actual_qty != quantity:
            logger.warning(f"SELL {symbol}: requested {quantity}, only {actual_qty} held — clamping")
            quantity = actual_qty
