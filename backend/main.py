...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.warning(f"Zebu contract load failed — on-demand SearchScrip will handle resolution: {e}")
...
async def websocket_endpoint(websocket: WebSocket, client_id: str = None):
    ...
    if token:
        try:
            ...
            logger.warning(f"WebSocket token verification failed for {connection_id}: {e}")
