...
async def place_order(
    ...
    if not quote or not quote.get("price"):
        ...
        if client_price and client_price > 0:
            ...
            quote = {"price": client_price, "name": symbol.replace(".NS", "")}
        else:
            return {
                "success": False,
    ...
    if side == "SELL" and product_type != "CNC":
        ...
        if not has_holdings:
            margin_required = total_cost / Decimal("5") if product_type == "MIS" else total_cost
            if margin_required > portfolio.available_capital:
                return {
                    "success": False,
                    "error": f"Insufficient margin for short sell. Required: ₹{margin_required:,.2f}, Available: ₹{portfolio.available_capital:,.2f}",
                }
    ...
    if side == "BUY":
        ...
        if holding and holding.quantity >= quantity:
            ...
            if holding and holding.quantity > 0:
                ...
                sell_pnl = (price - holding.avg_price) * close_qty
                portfolio.available_capital += price * close_qty
                portfolio.total_invested -= holding.avg_price * close_qty
                portfolio.total_pnl += sell_pnl
                await db.delete(holding)
                holding = None

            # Create short position — credit margin to available capital
            # MIS gets 5x leverage, so only 1/5 margin is blocked
            portfolio.available_capital += price * short_qty
