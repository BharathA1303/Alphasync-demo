...
const NOTIF_ICONS = {
    order_complete: { Icon: Check, color: 'text-emerald-600', bg: 'bg-emerald-500/20' },
    order_pending: { Icon: Clock, color: 'text-[#0EA5E9]', bg: 'bg-[#0EA5E9]/20' },
    order_rejected: { Icon: X, color: 'text-red-500', bg: 'bg-red-500/20' },
    market_open: { Icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-500/20' },
    market_close: { Icon: Info, color: 'text-blue-400', bg: 'bg-blue-500/20' },
    info: { Icon: Info, color: 'text-gray-400', bg: 'bg-gray-500/20' },
    warning: { Icon: AlertTriangle, color: 'text-[#0EA5E9]', bg: 'bg-[#0EA5E9]/20' },
};
...
function NotificationPanel({ notifications, onClear, onDismiss }) {
    return (
        <div className="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-slide-in">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200 dark:border-slate-700">
            ...
            <div className="max-h-80 overflow-y-auto">
                {notifications.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-10 gap-2">
                        <Bell className="w-8 h-8 text-gray-400 dark:text-slate-400" />
                        <span className="text-xs text-gray-500 dark:text-slate-500">No notifications</span>
                    </div>
                ) : (
                    notifications.map((n) => {
                        ...
                        return (
                            <div
                                key={n.id}
                                className="flex items-start gap-3 px-4 py-3 border-b border-gray-100 dark:border-slate-700/50 last:border-0 transition-colors hover:bg-gray-50 dark:hover:bg-slate-800"
                            >
                                <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5', cfg.bg)}>
                                    <Icon className={cn('w-4 h-4', cfg.color)} />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-xs text-gray-800 dark:text-slate-200 font-medium leading-snug">{n.title}</p>
                                    <p className="text-[11px] text-gray-500 dark:text-slate-400 mt-0.5 leading-snug">{n.message}</p>
                                    <span className="text-[10px] text-gray-400 dark:text-slate-500 mt-1 inline-block">{timeAgo(n.timestamp)}</span>
                                </div>
                                <button
                                    onClick={() => onDismiss(n.id)}
                                    className="text-gray-400 dark:text-slate-500 hover:text-gray-600 dark:hover:text-slate-300 transition-colors flex-shrink-0 mt-1"
                                    title="Dismiss"
                                >
...
export default function Navbar({ onMenuToggle }) {
    ...
    const statusColor = isMarketOpen ? 'bg-green-400' : 'bg-[#0EA5E9]';
    ...
    const wsColor = {
        ...
        connecting: 'bg-[#0EA5E9] animate-pulse',
    ...
    return (
        ...
        >
            ...
            <div className="absolute left-1/2 -translate-x-1/2 hidden md:block pointer-events-none">
                <span
                    className="bg-[#0EA5E9]/10 text-[#0EA5E9] border border-[#0EA5E9]/30 px-3 py-1 rounded-full
            ...
            <div className="flex items-center gap-3">
                <div className="relative hidden sm:block" ref={searchRef}>
                    ...
                    {showResults && (
                        <div
                            className="absolute top-full left-0 mt-1 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl shadow-2xl z-50 max-h-[320px] overflow-y-auto animate-slide-in"
                        ...
                        >
                            {searchResults.length > 0 ? searchResults.map((stock) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="flex-1 flex items-center gap-4 px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors text-left"
                                        >
                                            <div className="flex-1 min-w-0">
                                                <div className="text-sm font-semibold text-gray-900 dark:text-slate-100">
                                                    {stock.symbol.replace('.NS', '')}
                                                </div>
                                                <div className="text-xs text-gray-500 dark:text-slate-400 truncate">{stock.name}</div>
                                            </div>
                                            <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded flex-shrink-0 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300">
                                                {stock.exchange}
                                            </span>
                                        </button>
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                isWatchlisted
                                                    ? 'text-yellow-400 hover:text-gray-400 dark:hover:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700/30'
                                                    : 'text-gray-400 dark:text-slate-500 hover:text-yellow-400 hover:bg-yellow-500/10',
                                ...
                                <div className="px-4 py-3 text-center text-xs text-gray-500 dark:text-slate-400">
                                    No stocks found for "{searchQuery.trim()}"
                                </div>
                            )}
                        </div>
                    )}
